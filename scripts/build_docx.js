  ...
  sections: [
    {
      ...
      children: [
        ...
        p("Deep learning models for dermoscopic skin lesion classification increasingly incorporate explainable AI (XAI) techniques, yet most report explanation heatmaps only qualitatively, without verifying that explanations are anatomically faithful, and without connecting explanation reliability to the model's own predictive uncertainty. This paper proposes an uncertainty-aware, quantitatively validated explainable framework for multi-class skin lesion classification. A hybrid CNN-Transformer architecture (an EfficientNet-B0 convolutional stem feeding a lightweight Transformer encoder) is trained on HAM10000 dermoscopic images. Explanations are generated from two independent methods — Grad-CAM++ and Attention Rollout — and their agreement is measured quantitatively; explanation faithfulness is validated against ground-truth lesion segmentation masks via Intersection over Union (IoU) and the Pointing Game, rather than by visual inspection alone. Monte Carlo Dropout provides per-prediction epistemic uncertainty, which is fused with faithfulness and cross-method agreement into a single Trust Score. On a compute-constrained (CPU-only) feasibility-scale run — 799/199/201 train/val/test images, 20 training epochs — the proposed hybrid model reaches 77.1% accuracy, 0.472 balanced accuracy, 0.514 Cohen's kappa, and an Expected Calibration Error of just 0.021, winning 5 of 6 classification metrics outright against ResNet50, EfficientNet-B0, and ViT-Base baselines and losing only on macro-AUROC (0.861 vs. EfficientNet-B0's 0.908), while achieving a mean faithfulness IoU of 0.196 and Pointing Game accuracy of 48.8%. An ablation study at matched training scale shows that dermoscopic preprocessing (hair removal and CLAHE contrast normalization) improves accuracy, balanced accuracy, macro-F1, kappa, calibration, and both faithfulness metrics, trading off only macro-AUROC. All code, configurations, and results are released openly; the reduced experimental scale relative to the full study design — most consequentially, an 800-image local training subset that this study finds sets an accuracy ceiling largely independent of training epochs — is disclosed explicitly as a compute-driven limitation, with a full-scale reproduction path (GPU training on the complete 10,015-image dataset) provided."),
        ...
        p("Fig. 1 summarizes the full pipeline end to end: a dermoscopic image is preprocessed, passed through the hybrid CNN-Transformer to produce a prediction, and simultaneously routed through three parallel evaluation branches — explainability (Grad-CAM++ and Attention Rollout, cross-checked via agreement), faithfulness (both saliency maps compared against the ground-truth lesion mask), and uncertainty (MC-Dropout) — which are fused into the final Trust Score. The same preprocessed data also feeds three baseline architectures (ResNet50, EfficientNet-B0, ViT-Base) trained identically for comparison."),
        ...figure("fig0_methodology_diagram.png", 560, 336, "Fig. 1. End-to-end methodology: preprocessing, hybrid CNN-Transformer classification, and the parallel XAI / faithfulness / uncertainty branches fusing into the Trust Score."),
        ...
        p("T=10 passes are used in this study (vs. 20 in the full design, halved for CPU time). The predictive entropy of μ, normalized by log(num_classes), is used as the uncertainty term in the Trust Score."),
        ...
        p("Training: AdamW optimizer, learning rate 3×10⁻⁴, weight decay 1×10⁻⁴, cosine learning-rate schedule, batch size 16, 20 epochs, image resolution 160×160 (ViT-Base uses dynamic positional-embedding interpolation to accept this non-native resolution). This is a deliberate, disclosed reduction from the full design (50 epochs, 224×224, the complete 10,015-image dataset) made necessary by CPU-only compute availability for this study; see Section V. An earlier 6-epoch pilot run was superseded by this 20-epoch run once local training proved sustainable (Section V); only the 20-epoch numbers are reported here."),
        ...
        p(" "),
        p("Six metrics are reported deliberately rather than accuracy alone, because HAM10000's class imbalance (nv ≈ 68% of images) makes raw accuracy a weak signal on its own — all four models cluster within 5 points of each other on accuracy (72–77%) while fanning out far more on balanced accuracy, macro-F1, and kappa (Fig. 2), which is where the real separation between models shows up. The proposed hybrid model wins 5 of the 6 metrics outright — accuracy, balanced accuracy, macro-F1, kappa, and calibration, where its ECE of 0.021 is roughly five times lower (better) than any baseline. It loses only on macro-AUROC, where EfficientNet-B0 — the hybrid model's own CNN backbone — leads (0.908 vs. 0.861); Section IV-D isolates this trade-off to the Transformer stage specifically. ResNet50 and ViT-Base trail on every imbalance-sensitive metric despite reaching comparable raw accuracy, a gap between raw and balanced accuracy that is itself evidence they lean more heavily on the majority class under this study's training budget."),
        ...figure("fig1_classification_bars.png", 500, 281, "Fig. 2. Classification performance by model, grouped by metric (test set, n=201)."),
        p("Fig. 3 plots training and validation loss/accuracy across all 20 epochs for all four models. Two things are checked here that a final-epoch table cannot show. First, calibration: the hybrid model's dramatically lower ECE (0.021) is visible in its validation loss tracking closely with (and by epoch 15 slightly below) its training loss, whereas EfficientNet-B0 and ResNet50 show a widening train/validation gap from epoch 10 onward (e.g., EfficientNet-B0's training accuracy approaches 0.94–0.96 by epoch 20 while its validation accuracy plateaus near 0.75–0.78) — early-stage overfitting on the 800-image training set, reported openly rather than hidden behind a single final-epoch number. Second, convergence: no model's curves are fully flat by epoch 20, indicating the reported numbers remain a lower bound rather than a converged result."),
        ...figure("fig2_training_curves.png", 520, 272, "Fig. 3. Training/validation loss and accuracy over 20 epochs for all four models."),
        p("Fig. 4 shows the hybrid model's confusion matrix. The diagonal dominates for the majority class (127/137 nv images correct) but is weaker for minority classes (e.g., 2/7 akiec, 7/11 bcc). The off-diagonal mass is not scattered randomly: it concentrates on the akiec/bkl/mel → nv direction, most sharply for mel (11 of 17 mel images misclassified as nv, 64.7%) and, to a lesser extent, bkl (5 of 23, 21.7%). This tracks a clinically real ambiguity — melanoma, benign keratoses, and melanocytic nevi are notoriously difficult to distinguish visually even for trained dermatologists — rather than an arbitrary model failure mode, a distinction invisible from the aggregate metrics in Table I alone."),
        ...figure("fig3_confusion_matrix.png", 380, 345, "Fig. 4. Confusion matrix for the proposed hybrid model (test set, n=201)."),

        h("B. XAI Faithfulness and Cross-Method Agreement", HeadingLevel.HEADING_2),
        p("For the proposed hybrid model, mean faithfulness IoU (Grad-CAM++ vs. ground-truth lesion mask) was 0.196, and Pointing Game accuracy was 48.8% — i.e., the single most-activated pixel of the Grad-CAM++ map fell inside the true lesion region in nearly half of test images. Fig. 5 illustrates a representative, correctly-classified test case: both Grad-CAM++ and Attention Rollout concentrate activation within the true lesion boundary, but on visibly different sub-regions of it — a concrete example of exactly the kind of partial, method-dependent agreement that motivates measuring cross-method agreement quantitatively (Section III-D) rather than trusting a single method's heatmap by inspection. These aggregate faithfulness values are modest in absolute terms, consistent with a model trained on 800 images; they are reported without adjustment, as an honest baseline for future full-scale comparison, rather than presented as a mature clinical-grade result."),
        ...figure("fig5_xai_qualitative.png", 560, 142, "Fig. 5. Qualitative example (a correctly-classified test image) comparing Grad-CAM++ and Attention Rollout saliency against the ground-truth lesion mask."),

        h("C. Uncertainty and Trust Score", HeadingLevel.HEADING_2),
        p("The mean Trust Score across the test set was 0.297 (on a [0,1] scale). Because the Trust Score combines three quantities that are each still maturing at this training scale (calibration, faithfulness, and cross-method agreement), its absolute value should be read as a starting point for the metric's validation, not as a claim of high trustworthiness. Its value as a framework lies in making these three signals jointly visible and computable per-prediction — something the reviewed prior work (TIxAI, DermaScanAI, SkinSage XAI) does not do in combination."),

        h("D. Ablation Study", HeadingLevel.HEADING_2),
        p("Table II reports two of the six ablations from the study design (Section III), both run at the same 20-epoch training budget as the main results for a like-for-like comparison; the remaining four — multi-method XAI agreement's effect in isolation, uncertainty quantification on/off, focal loss vs. plain cross-entropy, and segmentation-guided cropping — were not executed under the CPU-only compute budget and are disclosed as future work rather than reported without evidence."),
        p("Table II. Ablation results", { bold: true, align: AlignmentType.CENTER }),
        makeTable(
          ["Ablation", "Variant", "Accuracy", "Bal. Acc.", "Macro F1", "Macro AUROC", "Faithfulness IoU"],
          [
          ...
          [16, 30, 10, 11, 11, 12, 12]
        ),
        p(" "),
        ...figure("fig4_ablation_bars.png", 560, 224, "Fig. 6. Ablation results: architecture (left) and preprocessing (right)."),
        p("Both ablations tell the same consistent story. The Transformer stage improves accuracy, balanced accuracy, macro-F1, and kappa over the CNN-only baseline, trading off macro-AUROC (0.861 vs. 0.908) — the same pattern visible between the hybrid model and EfficientNet-B0 in Table I, now isolated to the one architectural change that causes it. Preprocessing shows an identical trade-off shape: hair removal and CLAHE normalization improve accuracy, balanced accuracy, macro-F1, kappa, calibration (ECE 0.021 vs. 0.083), and both faithfulness metrics (IoU +0.056, Pointing Game +10.0 points), again at the cost of macro-AUROC (0.861 vs. 0.903). This is a markedly cleaner result than an earlier 6-epoch pilot of the same ablation, where balanced accuracy and macro-F1 had — within single-run noise — favored the no-preprocessing variant; at matched 20-epoch training scale, preprocessing's benefit is consistent across nearly every metric. Its effect on explanation quality specifically remains the clearest finding: faithfulness IoU and Pointing Game accuracy both improve substantially, consistent with the intuition that removing hair and illumination artifacts helps the model's activation patterns concentrate on the true lesion region rather than incidental image artifacts — a benefit that would not have been visible without the ground-truth-mask-anchored faithfulness metrics proposed in Section III-E."),
        ...
        p("• Data scale: 799/199/201 train/val/test images, drawn from the first 5,000 of HAM10000's 10,015 images, vs. the full dataset in the study's original design. This study finds this is the dominant constraint on accuracy specifically: increasing training from 6 to 20 epochs on the same 800-image subset substantially improved calibration, balance, and faithfulness (Section IV) but barely moved raw accuracy (76.1% to 77.1% for the hybrid model), indicating the accuracy ceiling is set by data scale, not training time."),
        p("• Training budget: 20 epochs vs. 50 in the full design; a single run with a fixed seed (no repeated runs/cross-validation, so no confidence intervals are reported). Training was twice interrupted by the local machine idle-sleeping before completing cleanly on a third attempt under a host-level keep-awake hold; this affected wall-clock time only, not the reported checkpoints, all of which come from runs that completed all 20 epochs without interruption."),
        ...
        p("None of these constraints were used to inflate or cherry-pick results: all reported numbers are the direct, unedited output of the released evaluation code (runs/eval_report_*.json), and the full experimental configuration is version-controlled alongside the results for exact reproducibility. A Google Colab notebook (notebooks/run_experiments.ipynb) is provided to reproduce this study at full scale on GPU hardware, which — given the finding above that data scale rather than epoch count is the binding constraint — is the most direct remaining path to closing the accuracy gap to full-scale literature results."),

        h("VI. Conclusion and Future Work", HeadingLevel.HEADING_1),
        p("This paper presented a hybrid CNN-Transformer framework for dermoscopic skin lesion classification that goes beyond typical single-method, qualitative XAI by (i) cross-validating two independent explanation methods against each other, (ii) quantitatively grounding explanation faithfulness in dermatologist-curated lesion segmentation masks rather than visual inspection, and (iii) fusing Monte Carlo Dropout uncertainty with explanation quality into a single, per-prediction Trust Score. Under a disclosed, compute-constrained pilot evaluation, the proposed architecture won 5 of 6 classification metrics against ResNet50, EfficientNet-B0, and ViT-Base baselines — most notably a roughly five-fold better calibration (ECE 0.021) — while trading off macro-AUROC to the Transformer stage specifically, a trade-off isolated and confirmed by the architecture ablation. The preprocessing ablation showed the same trade-off shape and, at matched training scale, improved explanation faithfulness alongside classification quality — a finding only visible because faithfulness was measured quantitatively rather than assumed. Comparing training budgets directly (6 vs. 20 epochs on the same data) further showed that this study's accuracy ceiling is set by training-set scale rather than epoch count, clarifying exactly what full-scale GPU training would need to change to close the remaining gap to the literature."),
